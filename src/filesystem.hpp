...
struct SuperBlock {
    ...
    inline void validate() const {
        if (this->signature != EXT2_SIGNATURE)
            PANIC_FORCABLE("Invalid EXT2 signature, you might be reading an EXT3/4 filesystem as EXT2, which is unsafe.");
        ...
        if (this->filesystem_state != FilesystemState::Clean)
            PANIC_FORCABLE("Filesystem is unclean/corrupted. Run a filesystem check and rerun this program.");

        if ((this->number_of_mounts_since_fsck+1 > this->number_of_mounts_since_fsck_allowed && this->number_of_mounts_since_fsck_allowed != 0) ||
                (time(NULL) - this->last_fsck_posix > this->time_between_forced_fsck_posix && this->time_between_forced_fsck_posix != 0))
...
#define SuperBlock_dbg(x) \
    ...
    x.total_inodes, \
    x.total_blocks, \
    x.blocks_reserved_for_superuser, \
    x.unallocated_blocks, \
    x.unallocated_inodes, \
    x.superblock_block_number, \
    x.block_size_logarythm, \
    x.fragment_size_logarythm, \
    x.blocks_in_block_group, \
    x.fragments_in_block_group, \
    x.inodes_in_block_group, \
    x.last_mount_time_posix, \
    x.last_write_time_posix, \
    x.number_of_mounts_since_fsck, \
    x.number_of_mounts_since_fsck_allowed, \
    x.signature, \
    (x.filesystem_state == FilesystemState::Clean) ? "clean" : "unclean", \
    static_cast<u32>(x.onerror_action), \
    x.version_minor, \
    x.last_fsck_posix, \
    x.time_between_forced_fsck_posix, \
    static_cast<u32>(x.os_id), \
    x.version_major, \
    x.user_id_for_reserved, \
    x.group_id_for_reserved
...
struct ExSuperBlock {
    ...
    inline bool has_write_feature(WriteFeatures feature) const {
        return (this->write_features & (u32)feature) != 0;
    }
...
#define ExSuperBlock_dbg(x) \
    ...
    "}\n", \
    x.first_non_reserved_inode, \
    x.inode_size, \
    x.block_group_of_superblock, \
   ...
   (x.has_write_feature(WriteFeatures::BinaryTree))               ? "BinaryTree"              : "", \
   x.fsid[0], x.fsid[1], x.fsid[2], x.fsid[3], x.fsid[4], x.fsid[5], x.fsid[6], x.fsid[7], x.fsid[8], x.fsid[9], x.fsid[10], x.fsid[11], x.fsid[12], x.fsid[13], x.fsid[14], x.fsid[15], \
   16, x.volume_name, \
   64, x.last_mount_path, \
   ...
   (x.is_compressed_with(CompressionAlgorithms::LZO))  ? "LZO"   : "", \
   x.blocks_to_preallocate_files, \
   x.blocks_to_preallocate_directories, \
   x.__unused, \
   x.journal_id[0], x.journal_id[1], x.journal_id[2], x.journal_id[3], x.journal_id[4], x.journal_id[5], x.journal_id[6], x.journal_id[7], x.journal_id[8], x.journal_id[9], x.journal_id[10], x.journal_id[11], x.journal_id[12], x.journal_id[13], x.journal_id[14], x.journal_id[15], \
   x.journal_inode, \
   x.journal_device, \
   x.head_of_orphan_inode_list

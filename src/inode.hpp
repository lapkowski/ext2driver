...
#define Inode_dbg(x) \
    ...
    "\tos_specyfic_two: %ud\n" \
    "}\n", \
    x.type_and_permissions, \
    x.user_id, \
    x.lower_size, \
    x.last_access_time, \
    x.creation_time, \
    x.last_modification_time, \
    x.deletion_time, \
    x.group_id, \
    x.hard_link_count, \
    x.disk_sector_count, \
    ...
    x.block_pointers[10], \
    x.block_pointers[11], \
    x.block_pointers[Inode::IND_BLOCK], \
    x.block_pointers[Inode::DIND_BLOCK], \
    x.block_pointers[Inode::TIND_BLOCK], \
    x.generation_number, \
    x.extended_attribute_block, \
    x.upper_size_or_dir_acl, \
    x.fragment_block_address, \
    x.os_specyfic_two
...
#define DirectoryEntry_dbg(fs, x) \
    ...
    x.total_entry_size, \
    x.lower_name_length, \
    x.upper_name_length_or_type, \
    x.lower_name_length + ((fs.has_required_feature(RequiredFeatures::DirectoryType)) ? 0 : (x.upper_name_length_or_type << 8)), x.name
...
public:
    ...
    InodeIterator(Filesystem* fs, Inode& inode, u8* buffer)
            : fs(fs), inode(inode), buffer(buffer) {
        this->increment();
    }
    ...
    inline bool operator==(const InodeIterator& other) const {
...
public:
    ...
    inline bool operator==(const DirInodeIterator& other) const {
        return this->counter == other.counter;
    }
...
private:
    DirInodeIterator(Inode& inode, const InodeIterator& iter) : fs(NULL), inode(inode), buffer(NULL), counter(-1), iter(iter.end()) {}
    void increment();
};
